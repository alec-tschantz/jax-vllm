...
- `prefill_step`: advances B prefilling slots by one `block_size` chunk each.
  ...
  block_tables, and returns logits for every query position.
...
def decoder_layer_prefill(
    d: DecoderLayer, hidden, cos, sin, layer, q_pos, block_tables, phys_blocks,
):
...
def prefill_step(
    ...
    for layer, layer_cache in zip(m.layers, cache.layers):
        hidden, layer_cache = decoder_layer_prefill(
            layer, hidden, cos, sin, layer_cache, q_pos, block_tables, phys_blocks
        )
        new_layers.append(layer_cache)
    hidden = rms_norm(m.norm, hidden)
    logits = linear(m.lm_head, hidden)
...
def decode_step_cb(
    ...
    for layer, layer_cache in zip(m.layers, cache.layers):
        hidden, layer_cache = decoder_layer_decode_cb(
            layer, hidden, cos, sin, layer_cache, positions, block_tables, phys_block, slot_in_block
        )
        new_layers.append(layer_cache)
    hidden = rms_norm(m.norm, hidden)
