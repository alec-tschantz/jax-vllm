...
# ---------- Data-only modules (eqx.Module is a frozen pytree dataclass) ----------
...
# ---------- Free functions (all compute lives here) ----------
...
def rms_norm(r: RMSNorm, x: Array) -> Array:
    ...
    inv_rms = lax.rsqrt(jnp.mean(x32 * x32, axis=-1, keepdims=True) + r.eps)
...
# Module-level switch so A/B benchmarks can toggle kernels between jit compiles.
# A/B on MI300X + ROCm JAX 0.9.2: einsum + jnp.repeat beats SDPA by ~5-13% at
# B=1..4 and ties at B=16. Keep einsum as default; SDPA kept for experiments.
...
def attention_kernel(
    ...
    # einsum path needs matched head counts.
...
def dense(d: Dense, x: Array) -> Array:
